...
- 8000 前端监听区新增业务机采集模式选择，开始监听时下发给在线业务机，采集范围仍以开始/结束监听批次为准。
- 系统 App 的 `面单解析` 页新增 `业务机采集规则`，可维护规则过滤采集模式使用的包含关键词和排除关键词。
- 新增业务机面单采集工具 V2，支持窗口配置服务端 URL、机器显示名和本地默认模式；启用后后台心跳，自动显示在服务端页面。

### 调整

- 业务机采集模式分为 `原始全量采集` 和 `规则过滤采集`。前者上传组件 `task.msg` 原文，后者提取打印文字并按系统采集规则过滤后上传。
- 业务机本地默认模式只作为兜底；前端页面开始监听时指定的模式优先。

## V7.9.2_20260605

### 修复

- 标题面单模式支持 `【标题】..., *1颜色分类: 规格, 鞋码/尺码: 尺码` 以及分号换行的同类格式，输出 `标题模式`。
- 修复带 `颜色分类/鞋码/尺码` 标签的长标题面单被旧店铺码标签解析抢先识别成 `店铺码模式` 的问题。
...
- 店铺直接打单模式不再把 `3.0/4.0` 或标题尾部规格片段当作鞋款；未命中鞋款规则时商品简称留空并标记为 `缺少鞋款规则`。
- 店铺直接打单模式支持 `ITEM_INFO` 后一行单独写尺码、数量写在 `ITEM_TOTAL_COUNT` 的原文格式。
