...
当前版本：`V7.9.3_20260607`

项目目录、模块边界和构建入口见 [`assembly_overview.md`](assembly_overview.md)。

## 本版目标

V7.9.3 新增采集模式管理：Web 后端和业务机采集客户端统一支持 `filtered`（筛选采集）和 `full`（全量采集），并把采集工具回传的真实打印原文持久化到 `data/waybill-monitor/collector_raw_records.jsonl`，Web 生成页可以查看和复制最近采集原文。
...
- Web 和业务机采集客户端共用 `/api/collector/config` 切换采集模式，默认 `filtered`。
- 服务端通过 `/api/collector/raw-records` 查看最近采集原文，原文数据保存在本地 `data/waybill-monitor/`，不提交到 GitHub。
...
- `src/core/collector_config.py`：采集模式枚举、配置校验和持久化。
- `src/core/collector_raw_records.py`：采集原文记录持久化和 Web 展示数据。
