...
- `src/five_field_normalizer.py`：五要素识别与标准行模块。
- `src/order_core.py`：订单整理核心逻辑。
- `src/app.py`：FastAPI Web 后端。
- `src/qt_admin.py`：订单整理管理系统。
- `src/qt_client.py`：一键整理订单客户端。
- `src/qt_web_console.py`：Web 服务控制台。
- `src/waybill_files.py`：服务端面单批次文件命名、原文 Excel 写出和记录去重。
- `src/waybill_collector_reader.py`：业务机打印组件数据库读取。
- `src/waybill_monitor.py`：旧兼容入口，新代码不再直接依赖。
- `src/waybill_raw_contract.py`：面单原文模板、字段和内部管线字段契约。
- `src/waybill_raw_pipeline.py`：订单系统侧的打印信息原文解析管线。
...
```bat
python src/qt_admin.py
python src/qt_client.py
python src/qt_web_console.py
```
